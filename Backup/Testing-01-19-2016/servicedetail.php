...
function getservicedetail($id)
{
    ...
    $con = DBconnect();
    $service = DBfetchone("SELECT shopid,name,shortdesc,price,funcdesc,intro,duration FROM Service WHERE serviceid = " . $id . ";", $con);
    if ($service == null) {
    ...
    $a = array("servicename" => $service["name"],"duration"=>$service["duration"], "shortdesc" => $service["shortdesc"], "price" => $service["price"], "funcdes" => $service["funcdesc"], "intro" => $service["intro"], "shopname" => $shop["name"], "latitude" => $shop["latitude"], "longtitude" => $shop["longtitude"], "phone" => $shop["phone"], "address" => $shop["address"], "pic" => $shop["pic"], "opentime" => $shop["opentime"], "closetime" => $shop["closetime"]);
    //echo json_encode($a);
    echo json_encode([
        'RespCode' => 111111,
        'RespContent' => [
